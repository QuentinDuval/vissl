...
"""
Helper functions used to auto-complete / infer elements of the configuration
based on other attributes inside the configuration
"""
...
def auto_complete_configuration(cfg: AttrDict):
    """
    ...
    if cfg.METERS is not None:
        infer_meter_names(cfg)
    add_dummy_parameters_if_frozen_model(cfg)
    infer_label_type(cfg)
    if cfg.OPTIMIZER.use_zero:
        convert_to_zero_optimizer(cfg)


def resolve_linear_schedule(cfg: AttrDict, param_schedulers: AttrDict):
    """
...
def infer_label_type(cfg: AttrDict):
    """
    In SSL, during pre-training we don't want to use annotated labels or during feature
    extraction, we don't have annotated labels for some datasets.

    In such cases, we set the label type to be just the image index in the dataset,
    unless the user has specifically provided "zero" as the label type, which is
...
def add_dummy_parameters_if_frozen_model(cfg: AttrDict):
    """
    ...
    extraction to speed up the extraction time.

    Since the model needs to be DDP for the distributed extraction, we need some dummy
    parameters in the model otherwise model can't be converted to DDP.

    So we attach some dummy head to the model.
    """
    world_size = cfg.DISTRIBUTED.NUM_NODES * cfg.DISTRIBUTED.NUM_PROC_PER_NODE
    if (
...
def convert_to_zero_optimizer(cfg):
    """
